resource "aws_vpc" "webapp" {
  ...
  tags = "${merge(merge(local.common_tags, map("Name", "webapp-${var.tier}")))}"
}

resource "aws_subnet" "public_subnet" {
  count             = "${length(data.aws_availability_zones.available.names)}"
  vpc_id            = "${aws_vpc.webapp.id}"
  cidr_block        = "${cidrsubnet(aws_vpc.webapp.cidr_block,6,0+count.index)}"
...
resource "aws_subnet" "private_subnet" {
  count             = "${length(data.aws_availability_zones.available.names)}"
  vpc_id            = "${aws_vpc.webapp.id}"
  cidr_block        = "${cidrsubnet(aws_vpc.webapp.cidr_block,6,7+count.index)}"
  availability_zone = "${element(data.aws_availability_zones.available.names, count.index)}"
